...
const verdictConfig = computed(() => {
    ...
    if (last.status >= 200 && last.status < 300) {
        if (redirectCount === 0) {
            return {
                ...
                icon: '✓',
                iconBg: 'bg-green-500',
                title: 'No redirects — direct response',
        ...
        return {
            ...
            icon: '✓',
            iconBg: 'bg-green-500',
            title: `${redirectCount} redirect${redirectCount > 1 ? 's' : ''} — ends OK`,
            titleCl: 'text-green-700 dark:text-green-300',
        }
    }
    if (last.status >= 300 && last.status < 400) {
        return {
            ...
            icon: '→',
            iconBg: 'bg-amber-500',
            title: 'Redirect chain not resolved',
            titleCl: 'text-amber-700 dark:text-amber-300',
        }
    }
    return {
        ...
        icon: '✗',
        iconBg: 'bg-rose-500',
        title: `Ends with HTTP ${last.status}`,
        titleCl: 'text-rose-700 dark:text-rose-300',
    }
})
    ...
    <AppLayout>
        <div class="max-w-2xl mx-auto px-4 sm:px-6 py-12">
            ...
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Redirect Checker</h1>
            ...
            <form @submit.prevent="runCheck" class="space-y-3 mb-8">
                ...
                <div class="flex items-center gap-2">
                    <label
                        for="redirect-user-agent"
                        class="text-xs text-gray-600 dark:text-gray-400 shrink-0 font-medium"
                        >User-Agent:</label
                    >
                    <select
            ...
            <div v-if="hops.length" class="space-y-4">
                ...
                <div class="ui-card overflow-hidden">
                    <div v-for="(hop, index) in hops" :key="index">
                        ...
                        <div
                            v-if="index > 0"
                            class="flex items-center gap-2 px-4 py-1.5 bg-gray-50 dark:bg-gray-900/60 border-y border-gray-100 dark:border-gray-800"
                        >
                        ...
                        <div>
                            ...
                            >
                                <!-- Step number -->
                                <span
                                    class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5"
                                >
                                    {{ index + 1 }}
                                </span>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-1.5">
                                        <span
                                            :class="[
                                                'inline-flex items-center px-2 py-0.5 rounded-md border text-xs font-bold font-mono',
                                                statusBadgeClass(hop.status),
                                            ]"
                                        >
                                            {{ hop.status }}
                                        </span>
                            ...
                            <!-- Expanded headers -->
                            <div
                                v-if="expandedHops.has(index) && hop.headers?.length"
                            ...
                            <div
                                v-else-if="expandedHops.has(index)"
                                class="border-t border-gray-100 dark:border-gray-800 px-4 py-3 text-xs text-gray-500 dark:text-gray-400 italic"
                            >
                ...
                >
                    ...
                    <p>
                        Each hop is fetched without auto-following redirects, so the full chain is captured
                        individually.
                    </p>
                    <p>
                        Click any hop to expand its raw HTTP response headers. The user-agent sent can be changed to

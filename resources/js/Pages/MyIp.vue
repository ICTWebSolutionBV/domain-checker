    ...
    <AppLayout>
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div class="text-center mb-8">
                <div
                    class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-emerald-600 mb-4 shadow-lg shadow-emerald-600/20"
                >
            ...
            <div class="ui-card overflow-hidden">
                <div class="p-8 flex flex-col items-center text-center gap-5">
                    ...
                    <template v-if="!ipDetecting && ipv4 && ipv6">
                        <div class="w-full max-w-sm space-y-2.5">
                            <!-- IPv4 row -->
                            <div
                                class="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50"
                            >
                                <span
                                    class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 w-9 shrink-0 text-left"
                                    >IPv4</span
                                >
                                <span
                                    class="font-mono text-sm font-bold text-gray-900 dark:text-white flex-1 text-left break-all"
                                    >{{ ipv4 }}</span
                                >
                                <div class="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <button
                                        @click="copyIp(ipv4)"
                                        type="button"
                                        class="flex items-center justify-center w-7 h-7 rounded-lg border border-emerald-200 dark:border-emerald-800 bg-white dark:bg-gray-900 hover:bg-emerald-50 dark:hover:bg-emerald-950 text-gray-500 dark:text-gray-400 transition-colors"
                                        title="Copy IPv4"
                                    >
                            ...
                            <!-- IPv6 row -->
                            <div
                                class="flex items-center gap-3 px-4 py-3 rounded-xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800/50"
                            >
                                <span
                                    class="text-xs font-semibold text-purple-600 dark:text-purple-400 w-9 shrink-0 text-left"
                                    >IPv6</span
                                >
                                <span
                                    class="font-mono text-xs font-bold text-gray-900 dark:text-white flex-1 text-left break-all"
                                    >{{ ipv6 }}</span
                                >
                                <div class="flex items-center gap-1.5 shrink-0">
                                    <span
                                    ...
                                    <button
                                        @click="copyIp(ipv6)"
                                        type="button"
                                        class="flex items-center justify-center w-7 h-7 rounded-lg border border-purple-200 dark:border-purple-800 bg-white dark:bg-gray-900 hover:bg-purple-50 dark:hover:bg-purple-950 text-gray-500 dark:text-gray-400 transition-colors"
                                        title="Copy IPv6"
                                    >
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                            <span
                                ...
                                :class="isIPv6 ? 'bg-purple-500' : 'bg-emerald-500'"
                            />
                    ...
                    <template v-else>
                        ...
                        <div class="flex items-center gap-3">
                            <span
                                class="font-mono text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 dark:text-white break-all"
                            >
                                {{ ip || '—' }}
                            </span>
                            <button
                ...
                <div v-if="mapSrc" class="border-t border-gray-200 dark:border-gray-800">
                    <iframe :src="mapSrc" class="w-full h-64 border-0" loading="lazy" referrerpolicy="no-referrer" />
                </div>
            </div>
            ...
            <div
                v-if="result?.private"
                class="mt-6 rounded-2xl border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 p-5 flex items-start gap-3"
            >
            ...
            <div v-if="result && !result.private" class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="ui-card p-5">
                    <div class="ui-section-title flex items-center gap-2 mb-4"><MapPin class="w-4 h-4" /> Location</div>
                    <dl class="space-y-2.5 text-sm">
                        ...
                        <div v-if="result.lat !== null && result.lon !== null" class="flex justify-between gap-4">
                            ...
                            <dd class="text-gray-900 dark:text-white text-right font-mono text-xs">
                ...
                <!-- Network -->
                <div class="ui-card p-5">
                    <div class="ui-section-title flex items-center gap-2 mb-4"><Network class="w-4 h-4" /> Network</div>
                    <dl class="space-y-2.5 text-sm">
                        <div class="flex justify-between gap-4">
                        ...
                        <div class="flex justify-between gap-4">
                            ...
                            <dd class="text-gray-900 dark:text-white text-right font-mono text-xs break-all">
                                {{ result.as || '—' }}
                            </dd>
                        </div>
                        <div v-if="result.reverse_dns" class="flex justify-between gap-4">
                            <dt class="text-gray-500 dark:text-gray-400">Reverse DNS</dt>
                            <dd class="text-gray-900 dark:text-white text-right break-all text-xs">
                                {{ result.reverse_dns }}
                            </dd>
                        </div>
                        <div class="flex justify-between gap-4">
                ...
                <div class="ui-card p-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                            <div
                                class="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-950 flex items-center justify-center shrink-0"
                            >
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="text-sm font-semibold text-gray-900 dark:text-white truncate">
                                    {{ browserInfo.browser }}
                                    <span
                                        v-if="browserInfo.version"
                                        class="font-normal text-gray-500 dark:text-gray-400"
                                    >
                                        {{ browserInfo.version }}</span
                                    >
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                            <div
                                class="w-9 h-9 rounded-lg bg-violet-100 dark:bg-violet-950 flex items-center justify-center shrink-0"
                            >
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                            <div
                                class="w-9 h-9 rounded-lg bg-teal-100 dark:bg-teal-950 flex items-center justify-center shrink-0"
                            >
                                <component
                                    :is="
                    ...
                    <div
                        class="mt-3 px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 text-xs font-mono text-gray-500 dark:text-gray-400 break-all"
                    >
            ...
            <!-- Info note -->
            <div
                class="mt-8 ui-panel-muted dark:bg-gray-900/50 rounded-2xl p-5 text-sm text-gray-600 dark:text-gray-400"
            >
                ...
                <p>
                    ...
                    <Link :href="route('ip')" class="text-indigo-600 dark:text-indigo-400 hover:underline"
                        >Use IP Lookup →</Link
                    >
                </p>
            </div>
